...
class TradingPartner(Base):
    ...
    # Customer-master fields synced from SAP (POST /partners/sync) — informational,
    # never used for integration routing (that's source_channel/gmail_label/api_config above).
...
class MaterialMaster(Base):
    ...
    frgn_name: Mapped[str | None] = mapped_column(String(500))
    hsn_code: Mapped[str | None] = mapped_column(String(10))
    gst_rate: Mapped[float | None] = mapped_column(Numeric(5, 2))
    itms_grp_cod: Mapped[int | None] = mapped_column(Integer)
    ...
    sales_uom: Mapped[str | None] = mapped_column(String(20))
    vat_group_purchase: Mapped[str | None] = mapped_column(String(20))
    vat_group_sales: Mapped[str | None] = mapped_column(String(20))
    ...
    frozen_for: Mapped[bool] = mapped_column(Boolean, nullable=False, default=False)
    is_active: Mapped[bool] = mapped_column(Boolean, nullable=False, default=True)
    created_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_utcnow)
    updated_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False, default=_utcnow, onupdate=_utcnow)
    deleted_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True))
...
class ShipToMapping(Base):
    ...
    id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), primary_key=True, default=uuid.uuid4)
    trading_partner_id: Mapped[uuid.UUID] = mapped_column(UUID(as_uuid=True), ForeignKey("trading_partners.id"), nullable=False)
    buyer_whs_code: Mapped[str] = mapped_column(String(100), nullable=False)
    ...
    # `state` is required for CGST/SGST vs IGST determination (CLAUDE.md section 8).
